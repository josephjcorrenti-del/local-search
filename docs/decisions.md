...
- No historical cleanup unless it is a complete delete.
- No sacred information.
...
[ ] rename schema/code references from path to index_path
...
[ ] rename sources.path to sources.index_path
[ ] rename documents.path to documents.index_path
[ ] rename chunks_fts.path to chunks_fts.index_path
[ ] update storage helpers for index_path
[ ] update tests for index_path
...
[ ] validate artifact path is a file
[ ] parse artifact JSON
[ ] support ollama_workbench web artifact JSON
[ ] extract url when present
[ ] extract title when present
[ ] extract content_text when present
[ ] store artifact path as index_path
...
[ ] add pytest happy path for index-web-artifact
[ ] add pytest invalid JSON test
[ ] add pytest missing content_text test
[ ] add bash smoke test for index-web-artifact + search
...
[ ] return index_path when available
...
[ ] need to be able to local_search <internet search> 
